...
Very similar to Gmarket (same parent company eBay Korea).
"""
...
class AuctionAdapter(BaseScraperAdapter):
    ...
    DEALS_URL = "https://www.auction.co.kr/AllKill/"
    WAIT_SELECTOR = ".allkill-list, .deal-item, .box__component"
    RATE_LIMIT_RPM = 15

    def __init__(self):
        """Initialize Auction adapter."""
        super().__init__()
    ...
    async def fetch_deals(self, category: Optional[str] = None) -> List[NormalizedDeal]:
        ...
        Args:
            category: Optional category filter (not used for Auction)
        ...
        try:
            self.logger.info("fetching_auction_deals", url=self.DEALS_URL)
            ...
            # Parse deal cards - multiple selector strategies
            deal_cards = (
                soup.select(".box__component") or
                soup.select(".deal-item") or
                soup.select(".box__item-container") or
                soup.select(".component--item_card")
            )
            ...
            self.logger.info("fetched_auction_deals", count=len(deals))
            return deals
    ...
    def _parse_deal_card(self, card) -> Optional[NormalizedDeal]:
        ...
        try:
            ...
            # Extract product ID from URL (itemno parameter or path)
            import re
            id_match = re.search(r"itemno=([A-Z0-9]+)", product_url) or re.search(r"/item/([A-Z0-9]+)", product_url)
            if not id_match:
                return None
            ...
            price_elem = (
                ...
                card.select_one(".price--seller strong")
            )
            if not price_elem:
                return None
            ...
            original_elem = (
                ...
                card.select_one(".price--original del")
            )
            if original_elem:
            ...
            discount_elem = (
                ...
                card.select_one(".text--discount em")
            )
            if discount_elem:
            ...
            if img_elem:
                ...
                if image_url and not image_url.startswith("http"):
                    image_url = f"https:{image_url}" if image_url.startswith("//") else f"https://www.auction.co.kr{image_url}"
    ...
    async def fetch_product_details(self, external_id: str) -> Optional[NormalizedProduct]:
        ...
        Args:
            external_id: Auction product ID (itemno)
        ...
        try:
            ...
            # Extract title
            title_elem = soup.select_one(".itemtit, .item_tit, h1.prod_tit")
            if not title_elem:
                return None
            title = title_elem.get_text(strip=True)

            # Extract current price
            price_elem = soup.select_one(".price_real strong, .price strong, .sale_price strong")
            if not price_elem:
                return None
            ...
            original_price = None
            original_elem = soup.select_one(".price_real del, .original_price del")
            if original_elem:
                original_text = original_elem.get_text(strip=True)
                original_price = PriceNormalizer.clean_price_string(original_text)
            ...
            # Extract image
            image_url = None
            img_elem = soup.select_one(".item_photo img, .prod_img img")
            if img_elem:
                image_url = img_elem.get("src") or img_elem.get("data-src")
                if image_url and not image_url.startswith("http"):
            ...
            brand = None
            brand_elem = soup.select_one(".brand, .seller_name")
            if brand_elem:
                brand = brand_elem.get_text(strip=True)

            # Extract description snippet
            description = None
            desc_elem = soup.select_one(".item_intro, .prod_info")
            if desc_elem:
